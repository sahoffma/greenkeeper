...
| **Priorität** | Hoch |
| **Erstellt** | 2026-07-29 |
| **Zuletzt geändert** | 2026-08-02 (Phase 7B Inventory Creation Persistenz) |
| **Verantwortlich** | — |
| **Verwandte Dokumente** | [GA-013](./ga-013.md); [DL-011](../decisions/dl-011.md); [DL-012](../decisions/dl-012.md); [DL-013](../decisions/dl-013.md); [DL-014](../decisions/dl-014.md); [DL-015](../decisions/dl-015.md); [DL-016](../decisions/dl-016.md); [DL-017](../decisions/dl-017.md); [DL-018](../decisions/dl-018.md); [DL-019](../decisions/dl-019.md); [DL-020](../decisions/dl-020.md); [DL-021](../decisions/dl-021.md); [DL-022](../decisions/dl-022.md); [DL-023](../decisions/dl-023.md); [DL-024](../decisions/dl-024.md); [DL-025](../decisions/dl-025.md); [GM-009](../model/gm-009.md); [DL-003](../decisions/dl-003.md); [GM-003](../model/gm-003.md); [GM-008](../model/gm-008.md); [CM-013](../playbook/conversation-model.md#cm-013--kontextbezogene-rückfragen-bei-der-erfassung); [product-governance.md](../product-governance.md); [greenkeeper-data-model.md](../greenkeeper-data-model.md) |
...
Product Profile speichert **Produktwissen** (wiederverwendbar, quellenbasiert). Persönlicher Bestand speichert **Besitz und Verbrauch** ([DL-012](../decisions/dl-012.md)); verbindliche Bestandsregeln (unveränderliche Bewegungen, keine Lagerortverwaltung in v1): [DL-019](../decisions/dl-019.md); Bindung an Produktversion, Produktform und Einheit: [DL-020](../decisions/dl-020.md); interne Basiseinheiten und Präzision: [DL-021](../decisions/dl-021.md); Inventory-Foundation-Grenzen: [DL-022](../decisions/dl-022.md); aggregierte Anzeige bei gebindebezogener Führung: [DL-023](../decisions/dl-023.md); physische Gebinde als separate Bestandsobjekte: [DL-024](../decisions/dl-024.md); Inventory-Core-Architektur und Persistenzgrenzen: [DL-025](../decisions/dl-025.md); explizite Bestandsanlage durch Nutzeraktion: [DL-026](../decisions/dl-026.md); Gebindeauflösung vor Bestandsanlage: [DL-027](../decisions/dl-027.md); Trennung von Gebindegröße und aktuellem Inhalt: [DL-028](../decisions/dl-028.md); Trennung von initialer Bestandsanlage und Bestandskorrektur: [DL-029](../decisions/dl-029.md). Die Enrichment Engine schreibt in das **Product Profile**; ein Product-Profile-Save erzeugt **keinen** automatischen Bestand — die Aufnahme in den Bestand erfolgt erst nach bestandener Readiness-Prüfung und **expliziter** Nutzeraktion zur Bestandsanlage ([DL-026](../decisions/dl-026.md)); mengenbasierte Eingaben erfordern **Gebindeauflösung mit Nutzerbestätigung** ([DL-027](../decisions/dl-027.md)); nominale **Gebindegröße** und **aktueller Inhalt** bleiben getrennt ([DL-028](../decisions/dl-028.md)).
...
| **7a** | Inventory Core Persistenz — Domain, Repository, atomare Movement-RPC ([DL-019](../decisions/dl-019.md)–[DL-028](../decisions/dl-028.md)) | ✅ umgesetzt — [§15](#15-phase-7a--inventory-core-persistenz-dünger) |
| **7b** | Inventory Creation Persistenz — atomar Item + initiales Movement, Creation Receipt, Request-Idempotenz ([DL-029](../decisions/dl-029.md)) | ✅ Persistenz/DB-Kern umgesetzt — [§16](#16-phase-7b--inventory-creation-persistenz-dünger); Nutzerworkflow, Gebindeauflösung, UI offen — [§16.11](#1611-abgrenzung--weiterhin-offen) |
...
### 11.14.1 Produktversion und Persistenzgrenze (Dünger)

Verbindliche fachliche Regel: [DL-018](../decisions/dl-018.md).

- Enrichment liefert die **aktuell erkannte und normalisierte** Herstellerdeklaration — nicht die alleinige Entscheidung über Produktidentität.
- Enrichment darf eine **bestehende Produktversion nicht überschreiben**.
- Vor späterer Persistenz muss geprüft werden, ob die Deklaration einer bekannten Produktversion entspricht:

| Ergebnis der Deklarationsprüfung | Verhalten |
|----------------------------------|-----------|
| Identische fachliche Deklaration | Bestehende Produktversion **wiederverwenden** |
| Abweichende fachliche Deklaration | **Neue** Produktversion — alte Version unverändert lassen |
| Unvollständige oder unsichere Deklaration | **Keine** automatische Versionsgleichheit |
| Abweichende Gebindegröße bei identischer Deklaration | Bestehende Produktversion **wiederverwenden**; Gebindegröße separat als Gebinde- beziehungsweise Bestandsinformation behandeln |

**Nicht** in Enrichment oder Persistenz vermischen:

- Produktidentität (`identityFingerprint`, Name, Barcode, …)
- Produktversion (normalisierte fachliche Deklaration)
- Gebindegröße, Verpackungsart, Restmenge (Bestand)

Composition Fingerprint für den Versionsvergleich: fachlich gefordert in DL-018 — **noch nicht** implementiert. Keine Behauptung einer bestehenden Persistenz- oder Save-Integration in dieser Spezifikationsstufe.
...
### 14.23 Runtime-Referenz Product-Profile-Save (Phase 6, HTTP-Grenze)

Produktiver Save-Endpunkt für bestätigte `intake_ready`-Jobs — **ohne** Bestand, UI oder Enrichment-Pipeline-Änderungen. Der Endpunkt delegiert ausschließlich an `saveConfirmedFertilizerProductProfile()` (Phase 5).

| Baustein | Pfad |
|----------|------|
| Netlify Function | `netlify/functions/fertilizer-product-profile-save.ts` |
| Öffentlicher API-Vertrag | `src/types/fertilizerProductProfileSave.ts` |
| Service (Job laden, Readiness, Save-Core) | `src/lib/fertilizerProductProfileSaveServerServiceCore.ts` |
| HTTP-Handler | `src/lib/fertilizerProductProfileSaveHandlerCore.ts` |
| Transport (Auth, Session, verbotene Client-Felder) | `src/lib/fertilizerProductProfileSaveTransportCore.ts` |
| Composition (Supabase, Repositories) | `src/lib/fertilizerProductProfileSaveCompositionCore.ts` |
| Save-Core (Projection, Fingerprint, Persistenz) | `src/lib/fertilizerProductProfileSaveCore.ts` |

**Öffentliche Operation:** `POST /.netlify/functions/fertilizer-product-profile-save`

**Request:** `{ enrichmentJobId, userConfirmed: true, idempotencyKey }` — keine clientseitigen Access-, Profil- oder Pipeline-Felder.

**Response:** `{ profile, reusedExistingVersion }` — öffentliches Profil-Mapping ohne Fingerprints, Provenance oder interne IDs.

Auth und Session-HMAC: dieselbe Infrastruktur wie Phase-4-Enrichment (`resolveFertilizerEnrichmentServerContexts`, signiertes Session-Cookie, Bearer-Vorrang).

---

## 15. Phase 7A — Inventory Core Persistenz (Dünger)

**Status:** ✅ technisch abgeschlossen (Runtime + Migration + Tests getrennt versioniert)

| Commit | Inhalt |
|--------|--------|
| `6081789` | Produktiver Inventory Core: Domain, Repository, Mapping, atomare PostgreSQL-RPC, Migration `20250805_fertilizer_inventory_core.sql` |
| `77c30ab` | Tests, Harnesses, statische Migrationsverifikation (`scripts/verify-fertilizer-inventory-core-migration.mjs`) |

Phase 7A implementiert **keinen** Nutzerworkflow zur Bestandsanlage, **keine** Gebindeauflösung, **keine** UI und **keine** öffentliche Bestandsanlage-API. Fachliche Grundlagen: [DL-019](../decisions/dl-019.md) bis [DL-028](../decisions/dl-028.md).

### 15.1 Zustandsmodell (Ist vs. offen)

| Zustand | Bedeutung |
|---------|-----------|
| **Fachlich entschieden** | DL-019 bis DL-028 — unverändert; dieses GA dokumentiert nur den technischen Ist-Stand |
| **Technisch in Phase 7A umgesetzt** | Inventory-Core-Typen, Validierung, Repository-Vertrag, persistentes Mapping, atomare Movement-RPC, additive Tabellenerweiterung |
| **Durch automatisierte Tests geprüft** | Inventory-Tests (117), Type-Test (12), Gesamtsuite zum Abschluss (1065), TypeScript-Typecheck, statische Migrationsverifikation (16/16) — **Stand zum Phase-7A-Abschluss**, keine dauerhafte Garantie für spätere Gesamtsuiten |
| **Noch nicht gegen reale PostgreSQL-/Supabase-Instanz verifiziert** | Konkurrenzverhalten — siehe [§15.9](#159-grenze-des-nachweises) |
| **Bewusst offen (Produkt-/Anwendungsschicht)** | Nutzerworkflow, Gebindeauflösung, UI, öffentliche Bestandsanlage-API — siehe [§16.11](#1611-abgrenzung--weiterhin-offen) |
| **Phase 7B Persistenz/DB-Kern** | Atomare Item+Movement-Anlage, Creation Receipts, Request-Idempotenz — [§16](#16-phase-7b--inventory-creation-persistenz-dünger) ✅ technisch abgeschlossen |

### 15.2 Inventory Item

Ein **Inventory Item** repräsentiert genau **ein physisches Gebinde** ([DL-024](../decisions/dl-024.md)).

| Eigenschaft | Ist-Stand Phase 7A |
|-------------|-------------------|
| Bindung | `saved_product_profile_id` → gespeicherte Produktversion (`product_profiles`, `profile_status = saved`, `source = enrichment`) |
| Nominale Gebindegröße | `package_size_value` + `package_size_unit` (Metadatum, **nicht** aktueller Inhalt — [DL-028](../decisions/dl-028.md)) |
| Basiseinheit | `base_unit`: `kg` (Granulat) oder `ml` (Flüssigkeit) — abgeleitet aus Produktform, nicht clientseitig überschreibbar ([DL-020](../decisions/dl-020.md), [DL-021](../decisions/dl-021.md)) |
| Zugriffskontext | `access_kind` + `user_id` **oder** `session_access_hash` (64-stelliger HMAC-Hex) |
| Verboten am Item | `currentQuantity`, `balance`, kopierte Produkt-/Nährstoffwahrheit ([DL-025](../decisions/dl-025.md), [DL-022](../decisions/dl-022.md)) |

**Persistenz:** additive Erweiterung von `public.fertilizer_containers` — **keine** parallele Inventory-Tabelle.

### 15.3 Inventory Movement

Ein **Inventory Movement** ist die **einzige** Quelle für Mengenänderungen ([DL-019](../decisions/dl-019.md)).

| Eigenschaft | Ist-Stand Phase 7A |
|-------------|-------------------|
| Menge | `quantity_delta` — signiert, **≠ 0**, max. **vier** Dezimalstellen, max. Betrag 100 000 |
| Einheit | exakt `item.base_unit` — **keine** kg↔ml-Konvertierung ([DL-021](../decisions/dl-021.md)) |
| Immutability | nach Insert weder UPDATE noch DELETE (Trigger `prevent_fertilizer_stock_movement_*`) |
| Korrektur | ausschließlich durch **Gegenbewegung**, nicht durch Mutation |
| Saldo | **nicht** gespeichert — Summe der Core-Movements (`movement_at IS NOT NULL`) **nur** für das konkrete Item |

**Persistenz:** additive Erweiterung von `public.fertilizer_stock_movements`.

Core-Movements sind durch `access_kind`, `movement_at` und `inventory_idempotency_key` von Legacy-Capture-Bewegungen (`capture_idempotency_key`, ohne Core-Felder) unterscheidbar. Legacy-Pfade (`save_fertilizer_capture` u. a.) bleiben unverändert und werden **nicht** durch den Core-Pfad ersetzt.

### 15.4 Repository-Schicht

| Baustein | Pfad |
|----------|------|
| Domain-Typen | `src/types/fertilizerInventoryCore.ts` |
| Record-Validierung | `src/lib/fertilizerInventoryRecordValidationCore.ts` |
| Mengenlogik | `src/lib/fertilizerInventoryQuantityCore.ts` |
| Saldo (abgeleitet) | `src/lib/fertilizerInventoryBalanceCore.ts` |
| Movement-Regeln | `src/lib/fertilizerInventoryMovementCore.ts` |
| Idempotenz-Payload-Vergleich | `src/lib/fertilizerInventoryMovementIdempotencyCore.ts` |
| Repository-Vertrag + In-Memory | `src/lib/fertilizerInventoryRepositoryCore.ts` |
| Row-Mapping | `src/lib/fertilizerInventoryRepositoryMappingCore.ts` |
| Persistentes Repository | `src/lib/fertilizerInventoryRepositoryPersistentCore.ts` |
| RPC-Parameter und Fehlerabbildung | `src/lib/fertilizerInventoryAppendMovementRpcCore.ts` |

**Vertrag:** `appendMovement` für Core-Bewegungen ruft **ausschließlich** die PostgreSQL-RPC auf — **keine** Read-then-Insert-Sequenz, **keine** clientseitige Saldo-Wahrheit, **kein** Aufruf von `save_fertilizer_capture`.

`createInventoryItem` persistiert nur das Item — **ohne** atomare initiale Bewegung (Phase 7B).

### 15.5 Atomare Movement-RPC

**Funktion:** `public.append_fertilizer_inventory_core_movement`
**Migration:** `supabase/migrations/20250805_fertilizer_inventory_core.sql`
**Security:** `SECURITY DEFINER`, `set search_path = public`
**Berechtigung:** `GRANT EXECUTE` an `authenticated`, `service_role`

Ablauf innerhalb **einer** Datenbanktransaktion:

1. Aufrufer-Access-Parameter validieren; bei `auth.uid()` zusätzlich: kein Session-Scope, User-ID muss übereinstimmen
2. Inventory Item laden: `SELECT … FROM fertilizer_containers WHERE id = … **FOR UPDATE**`
3. Core-Item prüfen (`saved_product_profile_id`, `access_kind`, `base_unit` gesetzt)
4. Zugriff gegen **autoritative Item-Daten** prüfen — Movement-Scope wird aus dem Item übernommen, nicht aus frei gesetzten Movement-Feldern des Aufrufers
5. `p_unit` gegen `item.base_unit` prüfen
6. Menge validieren (≠ 0, max. vier Dezimalstellen, gültiger Bereich)
7. Scoped Idempotenz: vorhandenes Movement mit gleichem Schlüssel → Replay bei identischem Payload; Conflict bei abweichendem Payload; `unique_violation`-Handler bei Parallel-Insert
8. Saldo = `SUM(quantity_delta)` nur für `container_id = p_inventory_item_id` und `movement_at IS NOT NULL`
9. Ablehnung wenn `balance + p_quantity_delta < 0`
10. Insert des Movements mit Item-Access-Feldern; Rückgabe als `jsonb`

Der Aufrufer darf **nicht** als Wahrheit vorgeben: Produktbindung, Basiseinheit des Items, Zugriffseigentümer, bisheriger Bestand.

### 15.6 Fehlervertrag

| SQL/RPC | Repository (`FertilizerInventoryRepositoryError.code`) |
|---------|----------------------------------------------------------|
...
Zusätzlich im Repository-Vertrag: `invalid_stored_record`, `persistence_unavailable`. Rohe Datenbankfehlermeldungen sind **kein** fachlicher API-Vertrag — Mapping in `fertilizerInventoryAppendMovementRpcCore.ts`.

### 15.7 Core vs. Legacy

| Aspekt | Core-Pfad (Phase 7A) | Legacy-Pfad |
|--------|----------------------|-------------|
| Item-Anlage | `createInventoryItem` → `fertilizer_containers` mit `saved_product_profile_id` | Capture-Flow über Katalog/Candidate |
| Movement-Buchung | RPC `append_fertilizer_inventory_core_movement` | `save_fertilizer_capture` u. a. |
| Idempotenz | `inventory_idempotency_key` | `capture_idempotency_key` |
| Immutability-Trigger | schützt **alle** Movements vor UPDATE/DELETE | Legacy-Inserts ohne Core-Felder bleiben zulässig |

Product Profile Save ([§14.23](#1423-runtime-referenz-product-profile-save-phase-6-http-grenze)) erzeugt **keinen** Inventory Item ([DL-026](../decisions/dl-026.md)).

### 15.8 Test- und Verifikationsstand (Phase-7A-Abschluss)

| Prüfung | Ergebnis (Abschluss Phase 7A) |
|---------|-------------------------------|
| Inventory-Tests (`src/lib/fertilizerInventory*`) | 117 grün |
| Type-Test `fertilizerInventoryCore.test.ts` | 12 grün |
| Gesamtsuite | 1065 grün |
| TypeScript-Typecheck (`tsc -b`) | grün |
| Statische Migrationsverifikation | 16/16 grün |

Abgedeckte Regeln u. a. in `fertilizerInventoryAtomicMovementCore.test.ts`: parallele simulierte Abbuchungen, Idempotenz-Replay/-Conflict, Zugriffs- und Einheitenisolation, getrennte Gebinde, RPC-only im persistenten Repository, Immutability per Migration-String-Check.

### 15.9 Grenze des Nachweises

- Konkurrenzverhalten wurde über einen **serialisierenden Test-Harness** (`fertilizerInventoryAtomicMovementTestHarness.ts`) mit `Promise.all` **simuliert**.
- Es wurde **noch kein** echter Parallelitätstest gegen eine **reale PostgreSQL-/Supabase-Instanz** ausgeführt.
- Die SQL-Architektur verwendet `FOR UPDATE` und eine atomare Funktion — der reale DB-Parallelitätstest bleibt **vor produktiver Freigabe** erforderlich.
- **Nicht behaupten:** echte PostgreSQL-Parallelität sei bereits nachgewiesen.

### 15.10 Verweis Phase 7B

Der Persistenz-/DB-Kern für atomare Bestandsanlage (Item + initiales Movement, Creation Receipt, Request-Idempotenz) ist in [§16](#16-phase-7b--inventory-creation-persistenz-dünger) dokumentiert. Nutzerworkflow, Gebindeauflösung, UI und öffentliche Bestandsanlage-API bleiben **offen** ([§16.11](#1611-abgrenzung--weiterhin-offen)).

---

## 16. Phase 7B — Inventory Creation Persistenz (Dünger)

**Status:** ✅ technisch abgeschlossen (Persistenz/DB-Kern — Runtime, Migration, Tests getrennt versioniert)

| Commit | Inhalt |
|--------|--------|
| `86777bd` | Creation-Domain-Core (`fertilizerInventoryCreationCore.ts`) — Validierung, Normalisierung, Fingerprints, Fehlervertrag |
| `e87e6fe` | Atomare Creation-Persistenz: Migration `20250806_fertilizer_inventory_creation_core.sql`, RPC `create_fertilizer_inventory_core_from_confirmed_packages`, Repository-Erweiterung |
| `c598c98` | Creation-RPC-/Repository-Tests, statische Migrationsverifikation (`scripts/verify-fertilizer-inventory-creation-migration.mjs`) |
| `3631a27` | Echte PostgreSQL-/Supabase-Integrationstests, Test-Harness, DB-Schreibfreigabe über Prozessvariable |

Phase 7B in diesem Abschnitt umfasst **ausschließlich** den technischen Persistenz-/DB-Kern für atomare initiale Bestandsanlage. Sie umfasst **keinen** Nutzerworkflow, **keine** Gebindeauflösung, **keine** UI und **keine** öffentliche Bestandsanlage-HTTP-API. Fachliche Grundlagen: [DL-019](../decisions/dl-019.md) bis [DL-029](../decisions/dl-029.md).

### 16.1 Zustandsmodell (Ist vs. offen)

| Zustand | Bedeutung |
|---------|-----------|
| **Implementiert** | Domain-Core, Creation-RPC, Repository, Migration `20250806`, technische Receipt-Tabelle, Advisory Lock, scoped Unique-Indexes |
| **Gegen Unit-/Contract-Tests geprüft** | Creation-Domain (55), Creation-RPC (26), Repository-Creation (18), DB-Sicherheits-/Harness-Tests (10) |
| **Statisch an der Migration geprüft** | Creation-Migrationsverifikation 36/36; Inventory-Core-Migrationsverifikation 16/16 |
| **Gegen echte Remote-Dev-PostgreSQL-/Supabase-Instanz geprüft** | 10 DB-Integrationstests (Parallelität, Replay, Konflikt, Rollback, Scope, Konsistenz, Negativfälle) |
| **Noch nicht implementiert** | UI, HTTP-Endpunkt, Gebindeauflösung, Package-Auswahl/-Bestätigung, Verbrauch/Anwendung, Korrektur-UI, Bestandsübersicht, Movement-Edit/-Delete, automatische Anlage ohne explizite Nutzeraktion — [§16.11](#1611-abgrenzung--weiterhin-offen) |

### 16.2 Domain und Persistenz

Ein bestätigter **Creation-Vorgang** erzeugt atomar in **einer** PostgreSQL-Transaktion:

- genau **ein Inventory Item** pro explizit übergebenem physischem Package
- genau **ein positives initiales Movement** pro Item

| Regel | Ist-Stand |
|-------|-----------|
| Initiale Gründe | `initial_stock`, `purchase`, `gift_received` — **nicht** `inventory_correction` ([DL-029](../decisions/dl-029.md)) |
| Packages pro Vorgang | 1–20 explizite Packages; identische Packages bleiben **getrennte** physische Items |
| Nominale Gebindegröße vs. initialer Inhalt | getrennt; Initialmenge ≤ Package-Größe |
| Basiseinheit | granular → `kg`; liquid → `ml` — **keine** kg↔ml-Umrechnung ([DL-020](../decisions/dl-020.md), [DL-021](../decisions/dl-021.md)) |
| Dezimalstellen | max. **vier** |
| Gespeicherter Bestand | **keiner** — keine Balance-Spalte, kein `currentQuantity`, keine zweite Inventory-Wahrheit ([DL-019](../decisions/dl-019.md), [DL-028](../decisions/dl-028.md)) |
| Product-Profile-Wahrheit | Item kopiert **keine** Nährstoff- oder Produktfelder — nur `saved_product_profile_id` ([DL-025](../decisions/dl-025.md)) |

### 16.3 Creation Receipt

Tabelle: `public.fertilizer_inventory_creation_receipts`

| Eigenschaft | Bedeutung |
|-------------|-----------|
| Zweck | **Technische** Operations- und Idempotenzstruktur — **keine** Bestandswahrheit |
| Scope | `access_kind`, `user_id` oder `session_access_hash` |
| Idempotenz | scoped `idempotency_key` (User- bzw. Session-Unique-Index) |
| Fingerprint | `payload_fingerprint` — **serverseitig** SHA-256 aus normalisierten autoritativen Eingaben; Client liefert **keinen** autoritativen Fingerprint |
| Referenz | `saved_product_profile_id`, `creation_reason`, optional `source_event_ref` |
| Ergebnis | `result_jsonb` — deterministische Operation-, Item- und Movement-IDs |

Direkte Client-Schreibzugriffe auf Receipts sind **nicht** vorgesehen; Anlage ausschließlich über die Creation-RPC.

### 16.4 Idempotenz und Replay

| Szenario | Verhalten |
|----------|-----------|
| Gleicher Scope + gleicher Key + gleicher Fingerprint | Deterministischer Replay — **keine** neuen Items/Movements; dieselben Operation-, Item- und Movement-IDs |
| Gleicher Scope + gleicher Key + **anderer** Fingerprint | `INVENTORY_CREATION_IDEMPOTENCY_CONFLICT` — genau ein vollständiger Erfolg, kein Teilanlage des Konflikt-Payloads |
| Gleicher Key in **unterschiedlichen** Scopes | Getrennte Receipts und Creation-Vorgänge — keine scope-übergreifende Kollision |
| Movement-Idempotenzschlüssel | `inventory-create:<receipt-id>:<sequence-index>` — der **rohe Request-Key** ist **nicht** Bestandteil des Movement-Schlüssels |
| Lange gemeinsame Request-Key-Präfixe (≥ 240 Zeichen) | Getrennte Operationen — **keine** Movement-Key-Kollision durch Präfixüberlappung |

### 16.5 Parallelität und Transaktion

| Mechanismus | Ist-Stand |
|-------------|-----------|
| Transaktionsgrenze | Vollständiger Creation-Vorgang in **einer** PostgreSQL-Funktion — Fehler rollen Receipt, Items und Movements **vollständig** zurück |
| Advisory Lock | 64-Bit-Ableitung aus SHA-256 über `{ accessKind, scopeId, idempotencyKey }` — **nicht** `hashtext` |
| Endgültige Korrektheitsgrenze | Scoped Unique-Indexes auf Receipt-Idempotenz **und** Movement-Idempotenzschlüssel |
| Retry nach Rollback | Mit demselben Idempotenz-Key nach korrigiertem Payload **zulässig** |

### 16.6 Product Profile und Zugriff

| Prüfung | Ist-Stand |
|---------|-----------|
| Autoritative Quelle | Gespeichertes Product Profile wird serverseitig geladen |
| Readiness | `profile_status = 'saved'`, `source = 'enrichment'` |
| Basiseinheit | abgeleitet aus `product_form` (granular/liquid) |
| Scope | User- und Session-Scope **getrennt** geprüft; Receipt, Item und Movement erhalten **denselben** Zugriffsscope |
| Fremdzugriff | kontrolliert abgelehnt (`INVENTORY_CREATION_ACCESS_DENIED`) |

### 16.7 Runtime-Referenz

| Baustein | Pfad |
|----------|------|
| Creation-Domain-Core | `src/lib/fertilizerInventoryCreationCore.ts` |
| Creation-RPC-Core (Parameter, Mapping, Fehler) | `src/lib/fertilizerInventoryCreationRpcCore.ts` |
| Repository-Erweiterung | `src/lib/fertilizerInventoryRepositoryCore.ts`, `src/lib/fertilizerInventoryRepositoryPersistentCore.ts` |
| PostgreSQL-RPC | `public.create_fertilizer_inventory_core_from_confirmed_packages` |
| Migration | `supabase/migrations/20250806_fertilizer_inventory_creation_core.sql` |
| Statische Migrationsverifikation | `scripts/verify-fertilizer-inventory-creation-migration.mjs` |
| Echte DB-Integrationstests | `src/lib/fertilizerInventoryCreationDatabase.test.ts` |
| DB-Test-Harness | `src/lib/fertilizerInventoryCreationDatabaseTestHarness.ts` |
| DB-Verifikationswrapper | `scripts/verify-fertilizer-inventory-creation-database.mjs` |

**Berechtigung RPC:** `GRANT EXECUTE` an `authenticated`, `service_role` — `SECURITY DEFINER`, `set search_path = public`.

### 16.8 Echter Datenbanknachweis (Remote-Dev)

Folgende Szenarien wurden gegen eine **Remote-Dev-PostgreSQL-/Supabase-Instanz** (greenkeeper-dev, [GA-010](./ga-010.md)) geprüft — **nicht** gegen eine lokale Supabase-Instanz:

| Test | Szenario |
|------|----------|
| DB-1 | Paralleles identisches Replay — drei gleichzeitige RPC-Aufrufe → eine Receipt-Zeile, stabile IDs |
| DB-2 | Paralleler Payload-Konflikt — ein Erfolg, ein `INVENTORY_CREATION_IDEMPOTENCY_CONFLICT` |
| DB-3 | Zwei Idempotenz-Keys mit ≥ 240 identischen Präfixzeichen → getrennte Operationen |
| DB-4 | Deterministischer sequentieller Replay — keine neuen Rows |
| DB-5 | Gleicher Key in User- vs. Session-Scope → getrennte Receipts |
| DB-6 | Vollständiger Rollback nach begonnenem erstem Container-Insert; Retry danach erfolgreich |
| DB-7 | Direkte Receipt-/Item-/Movement-Konsistenzprüfung in PostgreSQL |
| DB-8 | Kontrollierte Negativfälle gegen echte DB |
| DB-9 | Persistentes Repository über die Creation-RPC |

### 16.9 Testsicherheit (DB-Schreibfreigabe)

Echte DB-Schreibtests sind **ausschließlich** durch die explizite Prozessumgebungsvariable freigegeben:
...
| Regel | Ist-Stand |
|-------|-----------|
| Freigabe | Nur exakter Prozesswert `'true'` — kein Truthy, kein `'TRUE'`, kein `'1'` |
| `.env.local` | Darf Verbindungsdaten liefern — **keine** Schreibfreigabe; ein gleichnamiger Dateiwert wird **ignoriert** |
| Standard-`npm test` ohne Prozessfreigabe | DB-Testdatei wird gefunden, 10 DB-Tests werden **sichtbar übersprungen**, keine DB-Schreibzugriffe |
| Verifikationswrapper | Setzt die Variable **nicht** selbst; bricht ohne Prozessfreigabe mit Exit ≠ 0 ab |
| Production-Refs | Blockiert |
| Cleanup | Scoped Bereinigung (`gk-7b-db`-Präfix); Secrets werden **nicht** geloggt |

**Gezielter DB-Lauf** (nur mit expliziter Prozessfreigabe):
...
```bash
ALLOW_SUPABASE_WRITE_TESTS=true npx vitest run src/lib/fertilizerInventoryCreationDatabase.test.ts
```

### 16.10 Test- und Verifikationsstand (Phase-7B-Abschluss — Snapshot)

| Prüfung | Ergebnis (Abschluss Phase 7B) |
|---------|-------------------------------|
| Creation-Domain-Core | 55 grün |
| Creation-RPC | 26 grün |
| Repository-Creation | 18 grün |
| DB-Sicherheits-/Harness-Tests | 10 grün |
| Echte DB-Integrationstests | 10 grün (nur mit Prozessfreigabe) |
| Gesamtsuite ohne Prozessfreigabe | 99 Testdateien — 98 bestanden, 1 übersprungen; 1170 Tests — 1160 bestanden, 10 übersprungen, 0 fehlgeschlagen |
| TypeScript-Typecheck (`tsc -b`) | grün |
| Produktionsbuild (`npm run build`) | grün |
| Statische Creation-Migrationsverifikation | 36/36 grün |
| Statische Inventory-Core-Migrationsverifikation | 16/16 grün |

Testzahlen sind **Snapshot des dokumentierten Abschlussstands** — keine dauerhafte Garantie für spätere Gesamtsuiten.

### 16.11 Abgrenzung — weiterhin offen

Phase 7B **Persistenz/DB-Kern** ist abgeschlossen. Folgendes ist **nicht** Teil dieses Abschlusses und **nicht** implementiert:

- UI für Bestandsanlage
- HTTP-Endpunkt für Bestandsanlage
- Gebindeauflösung aus Nutzereingaben ([DL-027](../decisions/dl-027.md))
- Package-Auswahl oder Package-Bestätigung durch den Nutzer
- Recognition-/Enrichment-Erweiterungen
- Verbrauch oder Anwendung
- Korrektur-UI (`inventory_correction` als spätere Gegenbewegung — nicht Creation)
- Bestandsübersicht oder Aggregations-UI
- Editieren oder Löschen von Movements
- automatische Inventory-Anlage ohne explizite Nutzeraktion
- Umrechnung zwischen Masse und Volumen
