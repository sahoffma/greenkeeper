...
export type FertilizerManufacturerDocumentClassification =
  | 'exact_match'
  | 'partial_match'
  | 'no_match'

export interface FertilizerManufacturerDocumentParsedNutrient {
...
export interface FertilizerManufacturerDocumentParseResult {
  ...
  nutrients: FertilizerManufacturerDocumentParsedNutrient[]
...
export class FertilizerManufacturerDocumentParserError extends Error {
...
function extractNpk(text: string): FertilizerManufacturerDocumentParseResult['npk'] {
  const match =
...
function classifyDocument(
  ...
  extracted: ReturnType<typeof extractDocumentIdentity>,
): FertilizerManufacturerDocumentClassification {
  ...
  if (!manufacturerMatches || !productMatches) {
    return 'no_match'
  }
...
export function parseFertilizerManufacturerDocumentText(
  ...
  const extractedIdentity = extractDocumentIdentity(trimmed)
  const classification = classifyDocument(trimmed, expectedIdentity, extractedIdentity)
  const npk = extractNpk(trimmed)
  const nutrients: FertilizerManufacturerDocumentParsedNutrient[] = []
  ...
  const variantMatched =
    classification === 'exact_match'
      ? true
      : expectedVariant.length === 0
        ? true
        : normalizeComparable(extractedIdentity.variant).replace(/\s+/g, '') === expectedVariant ||
          normalizeComparable(trimmed).includes(expectedVariant)
